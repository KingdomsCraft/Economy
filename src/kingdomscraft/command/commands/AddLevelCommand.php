...
use kingdomscraft\command\tasks\AddLevelCommandTask;
use kingdomscraft\Main;
use pocketmine\command\CommandSender;

class AddLevelCommand extends EconomyCommand {

	public function __construct(Main $plugin) {
//		$this->setPermission("economy.command.setgold");
		parent::__construct($plugin, "addlevel", "Give levels to a player", "/addlevel {player} {amount}", []);
	}
	...
	public function run(CommandSender $sender, array $args) {
		if(isset($args[1])) {
			...
			if(is_int($amount) and $amount >= 0) {
				$this->getPlugin()->getServer()->getScheduler()->scheduleAsyncTask(new AddLevelCommandTask($this->getPlugin()->getEconomy()->getProvider(), $name, $amount, $sender->getName()));
				return true;
			} else {
