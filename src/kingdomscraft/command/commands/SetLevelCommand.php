...
use kingdomscraft\command\tasks\SetLevelCommandTask;
use kingdomscraft\Main;
use pocketmine\command\CommandSender;

class SetLevelCommand extends EconomyCommand {

	public function __construct(Main $plugin) {
//		$this->setPermission("economy.command.setgold");
		parent::__construct($plugin, "setlevel", "Set a players level", "/setlevel {player} {amount}", []);
	}
	...
	public function run(CommandSender $sender, array $args) {
		if(isset($args[1])) {
			...
			if(is_int($amount) and $amount >= 0) {
				$this->getPlugin()->getServer()->getScheduler()->scheduleAsyncTask(new SetLevelCommandTask($this->getPlugin()->getEconomy()->getProvider(), $name, $amount, $sender->getName()));
				return true;
			} else {
