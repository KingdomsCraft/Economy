...
use kingdomscraft\command\tasks\TakeLevelCommandTask;
use kingdomscraft\Main;
use pocketmine\command\CommandSender;

class TakeLevelCommand extends EconomyCommand {

	public function __construct(Main $plugin) {
//		$this->setPermission("economy.command.setgold");
		parent::__construct($plugin, "takelevel", "Take levels from a player", "/takelevel {player} {amount}", []);
	}
	...
	public function run(CommandSender $sender, array $args) {
		if(isset($args[1])) {
			...
			if(is_int($amount) and $amount >= 0) {
				$this->getPlugin()->getServer()->getScheduler()->scheduleAsyncTask(new TakeLevelCommandTask($this->getPlugin()->getEconomy()->getProvider(), $name, $amount, $sender->getName()));
				return true;
			} else {
