...
use kingdomscraft\command\tasks\TakeXpCommandTask;
	...
	public function run(CommandSender $sender, array $args) {
		if(isset($args[1])) {
			...
			if(is_int($amount) and $amount >= 0) {
				$this->getPlugin()->getServer()->getScheduler()->scheduleAsyncTask(new TakeXpCommandTask($this->getPlugin()->getEconomy()->getProvider(), $name, $amount, $sender->getName()));
				return true;
			} else {
