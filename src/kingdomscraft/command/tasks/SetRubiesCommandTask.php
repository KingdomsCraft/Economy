...
class SetRubiesCommandTask extends SetRubiesTask {

	/** @var string */
	protected $sender;

	/**
	 * SetRubiesCommandTask constructor
	 *
		...
		if($plugin instanceof Main and $plugin->isEnabled()) {
			...
			switch((is_array($result) ? $result[0] : $result)) {
				case self::SUCCESS:
					...
					if($notify) $sender->sendMessage($plugin->getMessage("command.set-rubies-success", [$this->name, $this->amount]));
					$plugin->getLogger()->debug("Successfully completed SetRubiesTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::CONNECTION_ERROR:
					...
					$plugin->getLogger()->debug("Connection error while executing SetRubiesTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::MYSQLI_ERROR:
					...
					$plugin->getLogger()->debug("MySQL error while executing SetRubiesTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::NO_DATA:
					if($notify) $sender->sendMessage($plugin->getMessage("command.no-data", [$this->name]));
					$plugin->getLogger()->debug("Failed to execute SetRubiesTask on kingdomscraft_database for {$this->name} as they don't have any data");
					return;
			}
		} else {
			throw new PluginException("Attempted to execute SetRubiesTask while Economy plugin isn't loaded!");
		}	}
