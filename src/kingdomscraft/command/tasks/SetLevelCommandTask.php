...
class SetLevelCommandTask extends SetLevelTask {

	/** @var string */
	protected $sender;

	/**
	 * SetLevelCommandTask constructor
	 *
		...
		if($plugin instanceof Main and $plugin->isEnabled()) {
			...
			switch((is_array($result) ? $result[0] : $result)) {
				case self::SUCCESS:
					...
					if($notify) $sender->sendMessage($plugin->getMessage("command.set-level-success", [$this->name, $this->amount]));
