...
class TakeXpCommandTask extends TakeXpTask {

	/** @var string */
	protected $sender;

	/**
	 * TakeXpCommandTask constructor
	 *
		...
		if($plugin instanceof Main and $plugin->isEnabled()) {
			...
			switch((is_array($result) ? $result[0] : $result)) {
				case self::SUCCESS:
					...
					if($notify) $sender->sendMessage($plugin->getMessage("take-xp-success", [$this->amount, $this->name]));
