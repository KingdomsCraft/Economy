...
class TakeGoldCommandTask extends TakeGoldTask {

	/** @var string */
	protected $sender;

	/**
	 * TakeGoldCommandTask constructor
	 *
		...
		if($plugin instanceof Main and $plugin->isEnabled()) {
			...
			switch((is_array($result) ? $result[0] : $result)) {
				case self::SUCCESS:
					...
					if($notify) $sender->sendMessage($plugin->getMessage("command.take-gold-success", [$this->name, $this->amount]));
