...
class TakeLevelCommandTask extends TakeLevelTask {

	/** @var string */
	protected $sender;

	/**
	 * TakeLevelCommandTask constructor
	 *
		...
		if($plugin instanceof Main and $plugin->isEnabled()) {
			...
			switch((is_array($result) ? $result[0] : $result)) {
				case self::SUCCESS:
					...
					if($notify) $sender->sendMessage($plugin->getMessage("command.take-level-success", [$this->name, $this->amount]));
