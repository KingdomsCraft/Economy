	...
	public function onRun() {
		...
		if($result instanceof \mysqli_result) {
			...
			if(is_array($row)) {
				...
				$this->setResult(self::WRONG_FORMAT);
				return;
			}
		}
		...
		if($plugin instanceof Main and $plugin->isEnabled()) {
			...
			switch((is_array($result) ? $result[0] : $result)) {
				...
				case self::MYSQLI_ERROR:
					...
					return;
				case self::WRONG_FORMAT:
					$plugin->getLogger()->debug("Error while reading data in LoadTask on kingdomscraft_Economy database for {$this->name}");
					return;
				case self::NO_DATA:
