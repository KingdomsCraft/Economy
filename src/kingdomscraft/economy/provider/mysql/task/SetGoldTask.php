...
class SetGoldTask extends MySQLTask {
	...
	public function onRun() {
		...
		// Do the query
		$mysqli->query("UPDATE kingdomscraft_economy SET gold = GREATEST({$this->amount}, 0) WHERE username = '{$mysqli->escape_string($this->name)}'");
		...
		if($plugin instanceof Main and $plugin->isEnabled()) {
			...
			switch((is_array($result) ? $result[0] : $result)) {
				case self::SUCCESS:
					...
					$plugin->getLogger()->debug("Successfully completed SetGoldTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::CONNECTION_ERROR:
					$plugin->getLogger()->critical("Couldn't connect to kingdomscraft_database! Error: {$result[1]}");
					$plugin->getLogger()->debug("Connection error while executing SetGoldTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::MYSQLI_ERROR:
					$plugin->getLogger()->error("MySQL error while querying kingdomscraft_database! Error: {$result[1]}");
					$plugin->getLogger()->debug("MySQL error while executing SetGoldTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::NO_DATA:
					$plugin->getLogger()->debug("Failed to execute SetGoldTask on kingdomscraft_database for {$this->name} as they don't have any data");
					return;
			}
		} else {
			throw new PluginException("Attempted to execute SetGoldTask while Economy plugin isn't loaded!");
		}
	}
