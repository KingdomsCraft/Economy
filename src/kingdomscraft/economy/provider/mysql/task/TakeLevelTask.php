...
class TakeLevelTask extends MySQLTask {
	...
	public function onRun() {
		...
		// Do the query
		$mysqli->query("UPDATE kingdomscraft_economy SET xp_level = xp_level - {$this->amount} WHERE username = '{$mysqli->escape_string($this->name)}'");
			...
			switch((is_array($result) ? $result[0] : $result)) {
				case self::SUCCESS:
					...
					if($info instanceof AccountInfo) {
						$info->level -= $this->amount;
					}
					$plugin->getLogger()->debug("Successfully completed TakeLevelTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::CONNECTION_ERROR:
					$plugin->getLogger()->critical("Couldn't connect to kingdomscraft_database! Error: {$result[1]}");
					$plugin->getLogger()->debug("Connection error while executing TakeLevelTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::MYSQLI_ERROR:
					$plugin->getLogger()->error("MySQL error while querying kingdomscraft_database! Error: {$result[1]}");
					$plugin->getLogger()->debug("MySQL error while executing TakeLevelTask on kingdomscraft_economy database for {$this->name}");
					return;
				case self::NO_DATA:
					$plugin->getLogger()->debug("Failed to execute TakeLevelTask on kingdomscraft_database for {$this->name} as they don't have any data");
					return;
			}
		} else {
			throw new PluginException("Attempted to execute TakeLevelTask while Economy plugin isn't loaded!");
		}
	}
